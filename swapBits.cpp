...
unsigned int swapBits(unsigned int number, int p1, int p2, int n)
{
	...
	if (p1 < 0 || p2 < 0 || n < 0 || p1>32 || p2>32)
		return 0;
	unsigned int flag1 = (number >> p1) & ((1 << n) - 1);
	unsigned int flag2 = (number >> p2) & ((1 << n) - 1);
	unsigned int covert = (flag1 ^ flag2);
	covert = (covert << p1) | (covert << p2);
	unsigned int result;
	result = number ^ covert;
	return result;
	return 0;
}
